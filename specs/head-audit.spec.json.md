...
## Spec
The review routine as a machine-readable spec: change triggers (retrain, head-count change, d_model change, prune), calendar floor, reviewer roles with the independence clause, sampling rule (counts and sources), required per-head measurements with thresholds, and the attach-or-it-doesn't-count gate. This IS the spec-files export.
