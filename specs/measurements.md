...
## Spec
Each required per-head measurement defined once: mass-across-boundary, max cross-head similarity between L2-normalized flattened per-head maps (stack, matmul against the transpose, read the off-diagonal), per-head entropy versus uniform, ablation delta (zero one head before the concat) — with how to compute it and the threshold the routine flags on.
